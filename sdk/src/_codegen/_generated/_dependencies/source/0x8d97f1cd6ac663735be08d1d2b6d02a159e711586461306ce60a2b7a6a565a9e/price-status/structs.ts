...
export function isPriceStatus(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V1}::price_status::PriceStatus`; }
...
export class PriceStatus implements StructClass { __StructClass = true as const;

 static readonly $typeName = `${PKG_V1}::price_status::PriceStatus`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = PriceStatus.$typeName; readonly $fullTypeName: `${typeof PKG_V1}::price_status::PriceStatus`; readonly $typeArgs: []; readonly $isPhantom = PriceStatus.$isPhantom;

 readonly status: ToField<"u64">

 private constructor(typeArgs: [], fields: PriceStatusFields, ) { this.$fullTypeName = composeSuiType( PriceStatus.$typeName, ...typeArgs ) as `${typeof PKG_V1}::price_status::PriceStatus`; this.$typeArgs = typeArgs;

 this.status = fields.status; }

 static reified( ): PriceStatusReified { return { typeName: PriceStatus.$typeName, fullTypeName: composeSuiType( PriceStatus.$typeName, ...[] ) as `${typeof PKG_V1}::price_status::PriceStatus`, typeArgs: [ ] as [], isPhantom: PriceStatus.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => PriceStatus.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => PriceStatus.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => PriceStatus.fromBcs( data, ), bcs: PriceStatus.bcs, fromJSONField: (field: any) => PriceStatus.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => PriceStatus.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => PriceStatus.fromSuiParsedData( content, ), fromSuiObjectData: (content: SuiObjectData) => PriceStatus.fromSuiObjectData( content, ), fetch: async (client: SuiClient, id: string) => PriceStatus.fetch( client, id, ), new: ( fields: PriceStatusFields, ) => { return new PriceStatus( [], fields ) }, kind: "StructClassReified", } }
 ...
 static fromSuiObjectData( data: SuiObjectData ): PriceStatus { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isPriceStatus(data.bcs.type)) { throw new Error(`object at is not a PriceStatus object`); }

 return PriceStatus.fromBcs( fromB64(data.bcs.bcsBytes) ); } if (data.content) { return PriceStatus.fromSuiParsedData( data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }
