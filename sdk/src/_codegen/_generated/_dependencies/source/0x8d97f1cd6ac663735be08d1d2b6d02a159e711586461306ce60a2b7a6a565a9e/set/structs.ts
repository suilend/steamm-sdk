...
export class Set<A extends TypeArgument> implements StructClass { __StructClass = true as const;

 static readonly $typeName = `${PKG_V1}::set::Set`; static readonly $numTypeParams = 1; static readonly $isPhantom = [false,] as const;

 readonly $typeName = Set.$typeName; readonly $fullTypeName: `${typeof PKG_V1}::set::Set<${ToTypeStr<A>}>`; readonly $typeArgs: [ToTypeStr<A>]; readonly $isPhantom = Set.$isPhantom;

 readonly keys: ToField<Vector<A>>; readonly elems: ToField<Table<ToPhantom<A>, ToPhantom<Unit>>>

 private constructor(typeArgs: [ToTypeStr<A>], fields: SetFields<A>, ) { this.$fullTypeName = composeSuiType( Set.$typeName, ...typeArgs ) as `${typeof PKG_V1}::set::Set<${ToTypeStr<A>}>`; this.$typeArgs = typeArgs;

 this.keys = fields.keys;; this.elems = fields.elems; }

 static reified<A extends Reified<TypeArgument, any>>( A: A ): SetReified<ToTypeArgument<A>> { return { typeName: Set.$typeName, fullTypeName: composeSuiType( Set.$typeName, ...[extractType(A)] ) as `${typeof PKG_V1}::set::Set<${ToTypeStr<ToTypeArgument<A>>}>`, typeArgs: [ extractType(A) ] as [ToTypeStr<ToTypeArgument<A>>], isPhantom: Set.$isPhantom, reifiedTypeArgs: [A], fromFields: (fields: Record<string, any>) => Set.fromFields( A, fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => Set.fromFieldsWithTypes( A, item, ), fromBcs: (data: Uint8Array) => Set.fromBcs( A, data, ), bcs: Set.bcs(toBcs(A)), fromJSONField: (field: any) => Set.fromJSONField( A, field, ), fromJSON: (json: Record<string, any>) => Set.fromJSON( A, json, ), fromSuiParsedData: (content: SuiParsedData) => Set.fromSuiParsedData( A, content, ), fromSuiObjectData: (content: SuiObjectData) => Set.fromSuiObjectData( A, content, ), fetch: async (client: SuiClient, id: string) => Set.fetch( client, A, id, ), new: ( fields: SetFields<ToTypeArgument<A>>, ) => { return new Set( [extractType(A)], fields ) }, kind: "StructClassReified", } }
 ...
 static fromSuiObjectData<A extends Reified<TypeArgument, any>>( typeArg: A, data: SuiObjectData ): Set<ToTypeArgument<A>> { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isSet(data.bcs.type)) { throw new Error(`object at is not a Set object`); }
