...
export function isDataSource(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V1}::data_source::DataSource`; }
...
export class DataSource implements StructClass { __StructClass = true as const;

 static readonly $typeName = `${PKG_V1}::data_source::DataSource`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = DataSource.$typeName; readonly $fullTypeName: `${typeof PKG_V1}::data_source::DataSource`; readonly $typeArgs: []; readonly $isPhantom = DataSource.$isPhantom;

 readonly emitterChain: ToField<"u64">; readonly emitterAddress: ToField<ExternalAddress>

 private constructor(typeArgs: [], fields: DataSourceFields, ) { this.$fullTypeName = composeSuiType( DataSource.$typeName, ...typeArgs ) as `${typeof PKG_V1}::data_source::DataSource`; this.$typeArgs = typeArgs;

 this.emitterChain = fields.emitterChain;; this.emitterAddress = fields.emitterAddress; }

 static reified( ): DataSourceReified { return { typeName: DataSource.$typeName, fullTypeName: composeSuiType( DataSource.$typeName, ...[] ) as `${typeof PKG_V1}::data_source::DataSource`, typeArgs: [ ] as [], isPhantom: DataSource.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => DataSource.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => DataSource.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => DataSource.fromBcs( data, ), bcs: DataSource.bcs, fromJSONField: (field: any) => DataSource.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => DataSource.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => DataSource.fromSuiParsedData( content, ), fromSuiObjectData: (content: SuiObjectData) => DataSource.fromSuiObjectData( content, ), fetch: async (client: SuiClient, id: string) => DataSource.fetch( client, id, ), new: ( fields: DataSourceFields, ) => { return new DataSource( [], fields ) }, kind: "StructClassReified", } }
 ...
 static fromSuiObjectData( data: SuiObjectData ): DataSource { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isDataSource(data.bcs.type)) { throw new Error(`object at is not a DataSource object`); }

 return DataSource.fromBcs( fromB64(data.bcs.bcsBytes) ); } if (data.content) { return DataSource.fromSuiParsedData( data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }
