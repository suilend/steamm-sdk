...
export function isPythFeeRecipient(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V1}::set_fee_recipient::PythFeeRecipient`; }
...
export class PythFeeRecipient implements StructClass { __StructClass = true as const;

 static readonly $typeName = `${PKG_V1}::set_fee_recipient::PythFeeRecipient`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = PythFeeRecipient.$typeName; readonly $fullTypeName: `${typeof PKG_V1}::set_fee_recipient::PythFeeRecipient`; readonly $typeArgs: []; readonly $isPhantom = PythFeeRecipient.$isPhantom;

 readonly recipient: ToField<"address">

 private constructor(typeArgs: [], fields: PythFeeRecipientFields, ) { this.$fullTypeName = composeSuiType( PythFeeRecipient.$typeName, ...typeArgs ) as `${typeof PKG_V1}::set_fee_recipient::PythFeeRecipient`; this.$typeArgs = typeArgs;

 this.recipient = fields.recipient; }

 static reified( ): PythFeeRecipientReified { return { typeName: PythFeeRecipient.$typeName, fullTypeName: composeSuiType( PythFeeRecipient.$typeName, ...[] ) as `${typeof PKG_V1}::set_fee_recipient::PythFeeRecipient`, typeArgs: [ ] as [], isPhantom: PythFeeRecipient.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => PythFeeRecipient.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => PythFeeRecipient.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => PythFeeRecipient.fromBcs( data, ), bcs: PythFeeRecipient.bcs, fromJSONField: (field: any) => PythFeeRecipient.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => PythFeeRecipient.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => PythFeeRecipient.fromSuiParsedData( content, ), fromSuiObjectData: (content: SuiObjectData) => PythFeeRecipient.fromSuiObjectData( content, ), fetch: async (client: SuiClient, id: string) => PythFeeRecipient.fetch( client, id, ), new: ( fields: PythFeeRecipientFields, ) => { return new PythFeeRecipient( [], fields ) }, kind: "StructClassReified", } }
 ...
 static fromSuiObjectData( data: SuiObjectData ): PythFeeRecipient { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isPythFeeRecipient(data.bcs.type)) { throw new Error(`object at is not a PythFeeRecipient object`); }

 return PythFeeRecipient.fromBcs( fromB64(data.bcs.bcsBytes) ); } if (data.content) { return PythFeeRecipient.fromSuiParsedData( data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }
