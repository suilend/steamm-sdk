...
export function isWormholeVAAVerificationReceipt(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V1}::governance::WormholeVAAVerificationReceipt`; }
...
export class WormholeVAAVerificationReceipt implements StructClass { __StructClass = true as const;

 static readonly $typeName = `${PKG_V1}::governance::WormholeVAAVerificationReceipt`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = WormholeVAAVerificationReceipt.$typeName; readonly $fullTypeName: `${typeof PKG_V1}::governance::WormholeVAAVerificationReceipt`; readonly $typeArgs: []; readonly $isPhantom = WormholeVAAVerificationReceipt.$isPhantom;

 readonly payload: ToField<Vector<"u8">>; readonly digest: ToField<Bytes32>; readonly sequence: ToField<"u64">

 private constructor(typeArgs: [], fields: WormholeVAAVerificationReceiptFields, ) { this.$fullTypeName = composeSuiType( WormholeVAAVerificationReceipt.$typeName, ...typeArgs ) as `${typeof PKG_V1}::governance::WormholeVAAVerificationReceipt`; this.$typeArgs = typeArgs;

 this.payload = fields.payload;; this.digest = fields.digest;; this.sequence = fields.sequence; }

 static reified( ): WormholeVAAVerificationReceiptReified { return { typeName: WormholeVAAVerificationReceipt.$typeName, fullTypeName: composeSuiType( WormholeVAAVerificationReceipt.$typeName, ...[] ) as `${typeof PKG_V1}::governance::WormholeVAAVerificationReceipt`, typeArgs: [ ] as [], isPhantom: WormholeVAAVerificationReceipt.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => WormholeVAAVerificationReceipt.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => WormholeVAAVerificationReceipt.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => WormholeVAAVerificationReceipt.fromBcs( data, ), bcs: WormholeVAAVerificationReceipt.bcs, fromJSONField: (field: any) => WormholeVAAVerificationReceipt.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => WormholeVAAVerificationReceipt.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => WormholeVAAVerificationReceipt.fromSuiParsedData( content, ), fromSuiObjectData: (content: SuiObjectData) => WormholeVAAVerificationReceipt.fromSuiObjectData( content, ), fetch: async (client: SuiClient, id: string) => WormholeVAAVerificationReceipt.fetch( client, id, ), new: ( fields: WormholeVAAVerificationReceiptFields, ) => { return new WormholeVAAVerificationReceipt( [], fields ) }, kind: "StructClassReified", } }
 ...
 static fromSuiObjectData( data: SuiObjectData ): WormholeVAAVerificationReceipt { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isWormholeVAAVerificationReceipt(data.bcs.type)) { throw new Error(`object at is not a WormholeVAAVerificationReceipt object`); }

 return WormholeVAAVerificationReceipt.fromBcs( fromB64(data.bcs.bcsBytes) ); } if (data.content) { return WormholeVAAVerificationReceipt.fromSuiParsedData( data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }
