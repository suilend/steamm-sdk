...
export function isUpdateFee(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V1}::set_update_fee::UpdateFee`; }
...
export class UpdateFee implements StructClass { __StructClass = true as const;

 static readonly $typeName = `${PKG_V1}::set_update_fee::UpdateFee`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = UpdateFee.$typeName; readonly $fullTypeName: `${typeof PKG_V1}::set_update_fee::UpdateFee`; readonly $typeArgs: []; readonly $isPhantom = UpdateFee.$isPhantom;

 readonly mantissa: ToField<"u64">; readonly exponent: ToField<"u64">

 private constructor(typeArgs: [], fields: UpdateFeeFields, ) { this.$fullTypeName = composeSuiType( UpdateFee.$typeName, ...typeArgs ) as `${typeof PKG_V1}::set_update_fee::UpdateFee`; this.$typeArgs = typeArgs;

 this.mantissa = fields.mantissa;; this.exponent = fields.exponent; }

 static reified( ): UpdateFeeReified { return { typeName: UpdateFee.$typeName, fullTypeName: composeSuiType( UpdateFee.$typeName, ...[] ) as `${typeof PKG_V1}::set_update_fee::UpdateFee`, typeArgs: [ ] as [], isPhantom: UpdateFee.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => UpdateFee.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => UpdateFee.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => UpdateFee.fromBcs( data, ), bcs: UpdateFee.bcs, fromJSONField: (field: any) => UpdateFee.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => UpdateFee.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => UpdateFee.fromSuiParsedData( content, ), fromSuiObjectData: (content: SuiObjectData) => UpdateFee.fromSuiObjectData( content, ), fetch: async (client: SuiClient, id: string) => UpdateFee.fetch( client, id, ), new: ( fields: UpdateFeeFields, ) => { return new UpdateFee( [], fields ) }, kind: "StructClassReified", } }
 ...
 static fromSuiObjectData( data: SuiObjectData ): UpdateFee { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isUpdateFee(data.bcs.type)) { throw new Error(`object at is not a UpdateFee object`); }

 return UpdateFee.fromBcs( fromB64(data.bcs.bcsBytes) ); } if (data.content) { return UpdateFee.fromSuiParsedData( data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }
