...
export function isI64(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V1}::i64::I64`; }

export interface I64Fields { negative: ToField<"bool">; magnitude: ToField<"u64"> }

export type I64Reified = Reified< I64, I64Fields >;

export class I64 implements StructClass { __StructClass = true as const;

 static readonly $typeName = `${PKG_V1}::i64::I64`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = I64.$typeName; readonly $fullTypeName: `${typeof PKG_V1}::i64::I64`; readonly $typeArgs: []; readonly $isPhantom = I64.$isPhantom;

 readonly negative: ToField<"bool">; readonly magnitude: ToField<"u64">

 private constructor(typeArgs: [], fields: I64Fields, ) { this.$fullTypeName = composeSuiType( I64.$typeName, ...typeArgs ) as `${typeof PKG_V1}::i64::I64`; this.$typeArgs = typeArgs;

 this.negative = fields.negative;; this.magnitude = fields.magnitude; }

 static reified( ): I64Reified { return { typeName: I64.$typeName, fullTypeName: composeSuiType( I64.$typeName, ...[] ) as `${typeof PKG_V1}::i64::I64`, typeArgs: [ ] as [], isPhantom: I64.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => I64.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => I64.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => I64.fromBcs( data, ), bcs: I64.bcs, fromJSONField: (field: any) => I64.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => I64.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => I64.fromSuiParsedData( content, ), fromSuiObjectData: (content: SuiObjectData) => I64.fromSuiObjectData( content, ), fetch: async (client: SuiClient, id: string) => I64.fetch( client, id, ), new: ( fields: I64Fields, ) => { return new I64( [], fields ) }, kind: "StructClassReified", } }
 ...
 static fromSuiObjectData( data: SuiObjectData ): I64 { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isI64(data.bcs.type)) { throw new Error(`object at is not a I64 object`); }

 return I64.fromBcs( fromB64(data.bcs.bcsBytes) ); } if (data.content) { return I64.fromSuiParsedData( data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }
