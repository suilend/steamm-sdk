...
export function isLatestOnly(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V1}::state::LatestOnly`; }
...
export class LatestOnly implements StructClass { __StructClass = true as const;

 static readonly $typeName = `${PKG_V1}::state::LatestOnly`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = LatestOnly.$typeName; readonly $fullTypeName: `${typeof PKG_V1}::state::LatestOnly`; readonly $typeArgs: []; readonly $isPhantom = LatestOnly.$isPhantom;

 readonly dummyField: ToField<"bool">

 private constructor(typeArgs: [], fields: LatestOnlyFields, ) { this.$fullTypeName = composeSuiType( LatestOnly.$typeName, ...typeArgs ) as `${typeof PKG_V1}::state::LatestOnly`; this.$typeArgs = typeArgs;

 this.dummyField = fields.dummyField; }

 static reified( ): LatestOnlyReified { return { typeName: LatestOnly.$typeName, fullTypeName: composeSuiType( LatestOnly.$typeName, ...[] ) as `${typeof PKG_V1}::state::LatestOnly`, typeArgs: [ ] as [], isPhantom: LatestOnly.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => LatestOnly.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => LatestOnly.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => LatestOnly.fromBcs( data, ), bcs: LatestOnly.bcs, fromJSONField: (field: any) => LatestOnly.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => LatestOnly.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => LatestOnly.fromSuiParsedData( content, ), fromSuiObjectData: (content: SuiObjectData) => LatestOnly.fromSuiObjectData( content, ), fetch: async (client: SuiClient, id: string) => LatestOnly.fetch( client, id, ), new: ( fields: LatestOnlyFields, ) => { return new LatestOnly( [], fields ) }, kind: "StructClassReified", } }
 ...
 static fromSuiObjectData( data: SuiObjectData ): LatestOnly { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isLatestOnly(data.bcs.type)) { throw new Error(`object at is not a LatestOnly object`); }

 return LatestOnly.fromBcs( fromB64(data.bcs.bcsBytes) ); } if (data.content) { return LatestOnly.fromSuiParsedData( data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }
...
export function isCurrentDigest(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V1}::state::CurrentDigest`; }
...
export class CurrentDigest implements StructClass { __StructClass = true as const;

 static readonly $typeName = `${PKG_V1}::state::CurrentDigest`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = CurrentDigest.$typeName; readonly $fullTypeName: `${typeof PKG_V1}::state::CurrentDigest`; readonly $typeArgs: []; readonly $isPhantom = CurrentDigest.$isPhantom;

 readonly dummyField: ToField<"bool">

 private constructor(typeArgs: [], fields: CurrentDigestFields, ) { this.$fullTypeName = composeSuiType( CurrentDigest.$typeName, ...typeArgs ) as `${typeof PKG_V1}::state::CurrentDigest`; this.$typeArgs = typeArgs;

 this.dummyField = fields.dummyField; }

 static reified( ): CurrentDigestReified { return { typeName: CurrentDigest.$typeName, fullTypeName: composeSuiType( CurrentDigest.$typeName, ...[] ) as `${typeof PKG_V1}::state::CurrentDigest`, typeArgs: [ ] as [], isPhantom: CurrentDigest.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => CurrentDigest.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => CurrentDigest.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => CurrentDigest.fromBcs( data, ), bcs: CurrentDigest.bcs, fromJSONField: (field: any) => CurrentDigest.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => CurrentDigest.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => CurrentDigest.fromSuiParsedData( content, ), fromSuiObjectData: (content: SuiObjectData) => CurrentDigest.fromSuiObjectData( content, ), fetch: async (client: SuiClient, id: string) => CurrentDigest.fetch( client, id, ), new: ( fields: CurrentDigestFields, ) => { return new CurrentDigest( [], fields ) }, kind: "StructClassReified", } }
 ...
 static fromSuiObjectData( data: SuiObjectData ): CurrentDigest { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isCurrentDigest(data.bcs.type)) { throw new Error(`object at is not a CurrentDigest object`); }

 return CurrentDigest.fromBcs( fromB64(data.bcs.bcsBytes) ); } if (data.content) { return CurrentDigest.fromSuiParsedData( data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }
