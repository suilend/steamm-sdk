...
export function isGovernanceDataSource(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V1}::set_governance_data_source::GovernanceDataSource`; }
...
export class GovernanceDataSource implements StructClass { __StructClass = true as const;

 static readonly $typeName = `${PKG_V1}::set_governance_data_source::GovernanceDataSource`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = GovernanceDataSource.$typeName; readonly $fullTypeName: `${typeof PKG_V1}::set_governance_data_source::GovernanceDataSource`; readonly $typeArgs: []; readonly $isPhantom = GovernanceDataSource.$isPhantom;

 readonly emitterChainId: ToField<"u64">; readonly emitterAddress: ToField<ExternalAddress>; readonly initialSequence: ToField<"u64">

 private constructor(typeArgs: [], fields: GovernanceDataSourceFields, ) { this.$fullTypeName = composeSuiType( GovernanceDataSource.$typeName, ...typeArgs ) as `${typeof PKG_V1}::set_governance_data_source::GovernanceDataSource`; this.$typeArgs = typeArgs;
 ...
 static reified( ): GovernanceDataSourceReified { return { typeName: GovernanceDataSource.$typeName, fullTypeName: composeSuiType( GovernanceDataSource.$typeName, ...[] ) as `${typeof PKG_V1}::set_governance_data_source::GovernanceDataSource`, typeArgs: [ ] as [], isPhantom: GovernanceDataSource.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => GovernanceDataSource.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => GovernanceDataSource.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => GovernanceDataSource.fromBcs( data, ), bcs: GovernanceDataSource.bcs, fromJSONField: (field: any) => GovernanceDataSource.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => GovernanceDataSource.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => GovernanceDataSource.fromSuiParsedData( content, ), fromSuiObjectData: (content: SuiObjectData) => GovernanceDataSource.fromSuiObjectData( content, ), fetch: async (client: SuiClient, id: string) => GovernanceDataSource.fetch( client, id, ), new: ( fields: GovernanceDataSourceFields, ) => { return new GovernanceDataSource( [], fields ) }, kind: "StructClassReified", } }
 ...
 static fromSuiObjectData( data: SuiObjectData ): GovernanceDataSource { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isGovernanceDataSource(data.bcs.type)) { throw new Error(`object at is not a GovernanceDataSource object`); }

 return GovernanceDataSource.fromBcs( fromB64(data.bcs.bcsBytes) ); } if (data.content) { return GovernanceDataSource.fromSuiParsedData( data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }
