...
export function isGovernanceInstruction(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V1}::governance_instruction::GovernanceInstruction`; }
...
export class GovernanceInstruction implements StructClass { __StructClass = true as const;

 static readonly $typeName = `${PKG_V1}::governance_instruction::GovernanceInstruction`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = GovernanceInstruction.$typeName; readonly $fullTypeName: `${typeof PKG_V1}::governance_instruction::GovernanceInstruction`; readonly $typeArgs: []; readonly $isPhantom = GovernanceInstruction.$isPhantom;
 ...
 private constructor(typeArgs: [], fields: GovernanceInstructionFields, ) { this.$fullTypeName = composeSuiType( GovernanceInstruction.$typeName, ...typeArgs ) as `${typeof PKG_V1}::governance_instruction::GovernanceInstruction`; this.$typeArgs = typeArgs;

 this.module = fields.module;; this.action = fields.action;; this.targetChainId = fields.targetChainId;; this.payload = fields.payload; }

 static reified( ): GovernanceInstructionReified { return { typeName: GovernanceInstruction.$typeName, fullTypeName: composeSuiType( GovernanceInstruction.$typeName, ...[] ) as `${typeof PKG_V1}::governance_instruction::GovernanceInstruction`, typeArgs: [ ] as [], isPhantom: GovernanceInstruction.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => GovernanceInstruction.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => GovernanceInstruction.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => GovernanceInstruction.fromBcs( data, ), bcs: GovernanceInstruction.bcs, fromJSONField: (field: any) => GovernanceInstruction.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => GovernanceInstruction.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => GovernanceInstruction.fromSuiParsedData( content, ), fromSuiObjectData: (content: SuiObjectData) => GovernanceInstruction.fromSuiObjectData( content, ), fetch: async (client: SuiClient, id: string) => GovernanceInstruction.fetch( client, id, ), new: ( fields: GovernanceInstructionFields, ) => { return new GovernanceInstruction( [], fields ) }, kind: "StructClassReified", } }
 ...
 static fromSuiObjectData( data: SuiObjectData ): GovernanceInstruction { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isGovernanceInstruction(data.bcs.type)) { throw new Error(`object at is not a GovernanceInstruction object`); }

 return GovernanceInstruction.fromBcs( fromB64(data.bcs.bcsBytes) ); } if (data.content) { return GovernanceInstruction.fromSuiParsedData( data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }
