...
export class HotPotatoVector<T extends TypeArgument> implements StructClass { __StructClass = true as const;
 ...
 readonly $typeName = HotPotatoVector.$typeName; readonly $fullTypeName: `${typeof PKG_V1}::hot_potato_vector::HotPotatoVector<${ToTypeStr<T>}>`; readonly $typeArgs: [ToTypeStr<T>]; readonly $isPhantom = HotPotatoVector.$isPhantom;

 readonly contents: ToField<Vector<T>>

 private constructor(typeArgs: [ToTypeStr<T>], fields: HotPotatoVectorFields<T>, ) { this.$fullTypeName = composeSuiType( HotPotatoVector.$typeName, ...typeArgs ) as `${typeof PKG_V1}::hot_potato_vector::HotPotatoVector<${ToTypeStr<T>}>`; this.$typeArgs = typeArgs;

 this.contents = fields.contents; }

 static reified<T extends Reified<TypeArgument, any>>( T: T ): HotPotatoVectorReified<ToTypeArgument<T>> { return { typeName: HotPotatoVector.$typeName, fullTypeName: composeSuiType( HotPotatoVector.$typeName, ...[extractType(T)] ) as `${typeof PKG_V1}::hot_potato_vector::HotPotatoVector<${ToTypeStr<ToTypeArgument<T>>}>`, typeArgs: [ extractType(T) ] as [ToTypeStr<ToTypeArgument<T>>], isPhantom: HotPotatoVector.$isPhantom, reifiedTypeArgs: [T], fromFields: (fields: Record<string, any>) => HotPotatoVector.fromFields( T, fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => HotPotatoVector.fromFieldsWithTypes( T, item, ), fromBcs: (data: Uint8Array) => HotPotatoVector.fromBcs( T, data, ), bcs: HotPotatoVector.bcs(toBcs(T)), fromJSONField: (field: any) => HotPotatoVector.fromJSONField( T, field, ), fromJSON: (json: Record<string, any>) => HotPotatoVector.fromJSON( T, json, ), fromSuiParsedData: (content: SuiParsedData) => HotPotatoVector.fromSuiParsedData( T, content, ), fromSuiObjectData: (content: SuiObjectData) => HotPotatoVector.fromSuiObjectData( T, content, ), fetch: async (client: SuiClient, id: string) => HotPotatoVector.fetch( client, T, id, ), new: ( fields: HotPotatoVectorFields<ToTypeArgument<T>>, ) => { return new HotPotatoVector( [extractType(T)], fields ) }, kind: "StructClassReified", } }
 ...
 static fromSuiObjectData<T extends Reified<TypeArgument, any>>( typeArg: T, data: SuiObjectData ): HotPotatoVector<ToTypeArgument<T>> { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isHotPotatoVector(data.bcs.type)) { throw new Error(`object at is not a HotPotatoVector object`); }
