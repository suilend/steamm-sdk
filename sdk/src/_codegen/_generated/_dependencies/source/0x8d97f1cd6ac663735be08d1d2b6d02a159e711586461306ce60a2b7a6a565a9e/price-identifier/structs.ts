...
export function isPriceIdentifier(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V1}::price_identifier::PriceIdentifier`; }
...
export class PriceIdentifier implements StructClass { __StructClass = true as const;

 static readonly $typeName = `${PKG_V1}::price_identifier::PriceIdentifier`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = PriceIdentifier.$typeName; readonly $fullTypeName: `${typeof PKG_V1}::price_identifier::PriceIdentifier`; readonly $typeArgs: []; readonly $isPhantom = PriceIdentifier.$isPhantom;

 readonly bytes: ToField<Vector<"u8">>

 private constructor(typeArgs: [], fields: PriceIdentifierFields, ) { this.$fullTypeName = composeSuiType( PriceIdentifier.$typeName, ...typeArgs ) as `${typeof PKG_V1}::price_identifier::PriceIdentifier`; this.$typeArgs = typeArgs;

 this.bytes = fields.bytes; }

 static reified( ): PriceIdentifierReified { return { typeName: PriceIdentifier.$typeName, fullTypeName: composeSuiType( PriceIdentifier.$typeName, ...[] ) as `${typeof PKG_V1}::price_identifier::PriceIdentifier`, typeArgs: [ ] as [], isPhantom: PriceIdentifier.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => PriceIdentifier.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => PriceIdentifier.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => PriceIdentifier.fromBcs( data, ), bcs: PriceIdentifier.bcs, fromJSONField: (field: any) => PriceIdentifier.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => PriceIdentifier.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => PriceIdentifier.fromSuiParsedData( content, ), fromSuiObjectData: (content: SuiObjectData) => PriceIdentifier.fromSuiObjectData( content, ), fetch: async (client: SuiClient, id: string) => PriceIdentifier.fetch( client, id, ), new: ( fields: PriceIdentifierFields, ) => { return new PriceIdentifier( [], fields ) }, kind: "StructClassReified", } }
 ...
 static fromSuiObjectData( data: SuiObjectData ): PriceIdentifier { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isPriceIdentifier(data.bcs.type)) { throw new Error(`object at is not a PriceIdentifier object`); }

 return PriceIdentifier.fromBcs( fromB64(data.bcs.bcsBytes) ); } if (data.content) { return PriceIdentifier.fromSuiParsedData( data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }
