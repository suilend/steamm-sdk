...
export function isGovernanceAction(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V1}::governance_action::GovernanceAction`; }
...
export class GovernanceAction implements StructClass { __StructClass = true as const;

 static readonly $typeName = `${PKG_V1}::governance_action::GovernanceAction`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = GovernanceAction.$typeName; readonly $fullTypeName: `${typeof PKG_V1}::governance_action::GovernanceAction`; readonly $typeArgs: []; readonly $isPhantom = GovernanceAction.$isPhantom;

 readonly value: ToField<"u8">

 private constructor(typeArgs: [], fields: GovernanceActionFields, ) { this.$fullTypeName = composeSuiType( GovernanceAction.$typeName, ...typeArgs ) as `${typeof PKG_V1}::governance_action::GovernanceAction`; this.$typeArgs = typeArgs;

 this.value = fields.value; }

 static reified( ): GovernanceActionReified { return { typeName: GovernanceAction.$typeName, fullTypeName: composeSuiType( GovernanceAction.$typeName, ...[] ) as `${typeof PKG_V1}::governance_action::GovernanceAction`, typeArgs: [ ] as [], isPhantom: GovernanceAction.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => GovernanceAction.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => GovernanceAction.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => GovernanceAction.fromBcs( data, ), bcs: GovernanceAction.bcs, fromJSONField: (field: any) => GovernanceAction.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => GovernanceAction.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => GovernanceAction.fromSuiParsedData( content, ), fromSuiObjectData: (content: SuiObjectData) => GovernanceAction.fromSuiObjectData( content, ), fetch: async (client: SuiClient, id: string) => GovernanceAction.fetch( client, id, ), new: ( fields: GovernanceActionFields, ) => { return new GovernanceAction( [], fields ) }, kind: "StructClassReified", } }
 ...
 static fromSuiObjectData( data: SuiObjectData ): GovernanceAction { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isGovernanceAction(data.bcs.type)) { throw new Error(`object at is not a GovernanceAction object`); }

 return GovernanceAction.fromBcs( fromB64(data.bcs.bcsBytes) ); } if (data.content) { return GovernanceAction.fromSuiParsedData( data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }
