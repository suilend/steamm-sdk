...
export function isGuardianSignature(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V1}::guardian_signature::GuardianSignature`; }
...
export class GuardianSignature implements StructClass { __StructClass = true as const;

 static readonly $typeName = `${PKG_V1}::guardian_signature::GuardianSignature`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = GuardianSignature.$typeName; readonly $fullTypeName: `${typeof PKG_V1}::guardian_signature::GuardianSignature`; readonly $typeArgs: []; readonly $isPhantom = GuardianSignature.$isPhantom;

 readonly r: ToField<Bytes32>; readonly s: ToField<Bytes32>; readonly recoveryId: ToField<"u8">; readonly index: ToField<"u8">

 private constructor(typeArgs: [], fields: GuardianSignatureFields, ) { this.$fullTypeName = composeSuiType( GuardianSignature.$typeName, ...typeArgs ) as `${typeof PKG_V1}::guardian_signature::GuardianSignature`; this.$typeArgs = typeArgs;

 this.r = fields.r;; this.s = fields.s;; this.recoveryId = fields.recoveryId;; this.index = fields.index; }

 static reified( ): GuardianSignatureReified { return { typeName: GuardianSignature.$typeName, fullTypeName: composeSuiType( GuardianSignature.$typeName, ...[] ) as `${typeof PKG_V1}::guardian_signature::GuardianSignature`, typeArgs: [ ] as [], isPhantom: GuardianSignature.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => GuardianSignature.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => GuardianSignature.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => GuardianSignature.fromBcs( data, ), bcs: GuardianSignature.bcs, fromJSONField: (field: any) => GuardianSignature.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => GuardianSignature.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => GuardianSignature.fromSuiParsedData( content, ), fromSuiObjectData: (content: SuiObjectData) => GuardianSignature.fromSuiObjectData( content, ), fetch: async (client: SuiClient, id: string) => GuardianSignature.fetch( client, id, ), new: ( fields: GuardianSignatureFields, ) => { return new GuardianSignature( [], fields ) }, kind: "StructClassReified", } }
 ...
 static fromSuiObjectData( data: SuiObjectData ): GuardianSignature { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isGuardianSignature(data.bcs.type)) { throw new Error(`object at is not a GuardianSignature object`); }

 return GuardianSignature.fromBcs( fromB64(data.bcs.bcsBytes) ); } if (data.content) { return GuardianSignature.fromSuiParsedData( data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }
