...
export class Set<T extends PhantomTypeArgument> implements StructClass { __StructClass = true as const;

 static readonly $typeName = `${PKG_V1}::set::Set`; static readonly $numTypeParams = 1; static readonly $isPhantom = [true,] as const;

 readonly $typeName = Set.$typeName; readonly $fullTypeName: `${typeof PKG_V1}::set::Set<${PhantomToTypeStr<T>}>`; readonly $typeArgs: [PhantomToTypeStr<T>]; readonly $isPhantom = Set.$isPhantom;

 readonly items: ToField<Table<T, ToPhantom<Empty>>>

 private constructor(typeArgs: [PhantomToTypeStr<T>], fields: SetFields<T>, ) { this.$fullTypeName = composeSuiType( Set.$typeName, ...typeArgs ) as `${typeof PKG_V1}::set::Set<${PhantomToTypeStr<T>}>`; this.$typeArgs = typeArgs;

 this.items = fields.items; }

 static reified<T extends PhantomReified<PhantomTypeArgument>>( T: T ): SetReified<ToPhantomTypeArgument<T>> { return { typeName: Set.$typeName, fullTypeName: composeSuiType( Set.$typeName, ...[extractType(T)] ) as `${typeof PKG_V1}::set::Set<${PhantomToTypeStr<ToPhantomTypeArgument<T>>}>`, typeArgs: [ extractType(T) ] as [PhantomToTypeStr<ToPhantomTypeArgument<T>>], isPhantom: Set.$isPhantom, reifiedTypeArgs: [T], fromFields: (fields: Record<string, any>) => Set.fromFields( T, fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => Set.fromFieldsWithTypes( T, item, ), fromBcs: (data: Uint8Array) => Set.fromBcs( T, data, ), bcs: Set.bcs, fromJSONField: (field: any) => Set.fromJSONField( T, field, ), fromJSON: (json: Record<string, any>) => Set.fromJSON( T, json, ), fromSuiParsedData: (content: SuiParsedData) => Set.fromSuiParsedData( T, content, ), fromSuiObjectData: (content: SuiObjectData) => Set.fromSuiObjectData( T, content, ), fetch: async (client: SuiClient, id: string) => Set.fetch( client, T, id, ), new: ( fields: SetFields<ToPhantomTypeArgument<T>>, ) => { return new Set( [extractType(T)], fields ) }, kind: "StructClassReified", } }
 ...
 static fromSuiObjectData<T extends PhantomReified<PhantomTypeArgument>>( typeArg: T, data: SuiObjectData ): Set<ToPhantomTypeArgument<T>> { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isSet(data.bcs.type)) { throw new Error(`object at is not a Set object`); }
