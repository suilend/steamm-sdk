...
export function isFeeCollector(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V1}::fee_collector::FeeCollector`; }
...
export class FeeCollector implements StructClass { __StructClass = true as const;

 static readonly $typeName = `${PKG_V1}::fee_collector::FeeCollector`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = FeeCollector.$typeName; readonly $fullTypeName: `${typeof PKG_V1}::fee_collector::FeeCollector`; readonly $typeArgs: []; readonly $isPhantom = FeeCollector.$isPhantom;

 readonly feeAmount: ToField<"u64">; readonly balance: ToField<Balance<ToPhantom<SUI>>>

 private constructor(typeArgs: [], fields: FeeCollectorFields, ) { this.$fullTypeName = composeSuiType( FeeCollector.$typeName, ...typeArgs ) as `${typeof PKG_V1}::fee_collector::FeeCollector`; this.$typeArgs = typeArgs;

 this.feeAmount = fields.feeAmount;; this.balance = fields.balance; }

 static reified( ): FeeCollectorReified { return { typeName: FeeCollector.$typeName, fullTypeName: composeSuiType( FeeCollector.$typeName, ...[] ) as `${typeof PKG_V1}::fee_collector::FeeCollector`, typeArgs: [ ] as [], isPhantom: FeeCollector.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => FeeCollector.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => FeeCollector.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => FeeCollector.fromBcs( data, ), bcs: FeeCollector.bcs, fromJSONField: (field: any) => FeeCollector.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => FeeCollector.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => FeeCollector.fromSuiParsedData( content, ), fromSuiObjectData: (content: SuiObjectData) => FeeCollector.fromSuiObjectData( content, ), fetch: async (client: SuiClient, id: string) => FeeCollector.fetch( client, id, ), new: ( fields: FeeCollectorFields, ) => { return new FeeCollector( [], fields ) }, kind: "StructClassReified", } }
 ...
 static fromSuiObjectData( data: SuiObjectData ): FeeCollector { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isFeeCollector(data.bcs.type)) { throw new Error(`object at is not a FeeCollector object`); }

 return FeeCollector.fromBcs( fromB64(data.bcs.bcsBytes) ); } if (data.content) { return FeeCollector.fromSuiParsedData( data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }
