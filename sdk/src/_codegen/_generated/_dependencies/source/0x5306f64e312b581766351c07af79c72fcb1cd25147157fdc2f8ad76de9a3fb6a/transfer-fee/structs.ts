...
export function isGovernanceWitness(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V1}::transfer_fee::GovernanceWitness`; }
...
export class GovernanceWitness implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `${PKG_V1}::transfer_fee::GovernanceWitness`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = GovernanceWitness.$typeName; readonly $fullTypeName: `${typeof PKG_V1}::transfer_fee::GovernanceWitness`; readonly $typeArgs: []; readonly $isPhantom = GovernanceWitness.$isPhantom;

 readonly dummyField: ToField<"bool">

 private constructor(typeArgs: [], fields: GovernanceWitnessFields, ) { this.$fullTypeName = composeSuiType( GovernanceWitness.$typeName, ...typeArgs ) as `${typeof PKG_V1}::transfer_fee::GovernanceWitness`; this.$typeArgs = typeArgs;

 this.dummyField = fields.dummyField; }

 static reified( ): GovernanceWitnessReified { return { typeName: GovernanceWitness.$typeName, fullTypeName: composeSuiType( GovernanceWitness.$typeName, ...[] ) as `${typeof PKG_V1}::transfer_fee::GovernanceWitness`, typeArgs: [ ] as [], isPhantom: GovernanceWitness.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => GovernanceWitness.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => GovernanceWitness.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => GovernanceWitness.fromBcs( data, ), bcs: GovernanceWitness.bcs, fromJSONField: (field: any) => GovernanceWitness.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => GovernanceWitness.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => GovernanceWitness.fromSuiParsedData( content, ), fromSuiObjectData: (content: SuiObjectData) => GovernanceWitness.fromSuiObjectData( content, ), fetch: async (client: SuiClient, id: string) => GovernanceWitness.fetch( client, id, ), new: ( fields: GovernanceWitnessFields, ) => { return new GovernanceWitness( [], fields ) }, kind: "StructClassReified", } }
...
export function isTransferFee(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V1}::transfer_fee::TransferFee`; }
...
export class TransferFee implements StructClass { __StructClass = true as const;

 static readonly $typeName = `${PKG_V1}::transfer_fee::TransferFee`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = TransferFee.$typeName; readonly $fullTypeName: `${typeof PKG_V1}::transfer_fee::TransferFee`; readonly $typeArgs: []; readonly $isPhantom = TransferFee.$isPhantom;

 readonly amount: ToField<"u64">; readonly recipient: ToField<"address">

 private constructor(typeArgs: [], fields: TransferFeeFields, ) { this.$fullTypeName = composeSuiType( TransferFee.$typeName, ...typeArgs ) as `${typeof PKG_V1}::transfer_fee::TransferFee`; this.$typeArgs = typeArgs;

 this.amount = fields.amount;; this.recipient = fields.recipient; }

 static reified( ): TransferFeeReified { return { typeName: TransferFee.$typeName, fullTypeName: composeSuiType( TransferFee.$typeName, ...[] ) as `${typeof PKG_V1}::transfer_fee::TransferFee`, typeArgs: [ ] as [], isPhantom: TransferFee.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => TransferFee.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => TransferFee.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => TransferFee.fromBcs( data, ), bcs: TransferFee.bcs, fromJSONField: (field: any) => TransferFee.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => TransferFee.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => TransferFee.fromSuiParsedData( content, ), fromSuiObjectData: (content: SuiObjectData) => TransferFee.fromSuiObjectData( content, ), fetch: async (client: SuiClient, id: string) => TransferFee.fetch( client, id, ), new: ( fields: TransferFeeFields, ) => { return new TransferFee( [], fields ) }, kind: "StructClassReified", } }
 ...
 static fromSuiObjectData( data: SuiObjectData ): TransferFee { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isTransferFee(data.bcs.type)) { throw new Error(`object at is not a TransferFee object`); }

 return TransferFee.fromBcs( fromB64(data.bcs.bcsBytes) ); } if (data.content) { return TransferFee.fromSuiParsedData( data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }
