...
export function isGuardianSet(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V1}::guardian_set::GuardianSet`; }
...
export class GuardianSet implements StructClass { __StructClass = true as const;

 static readonly $typeName = `${PKG_V1}::guardian_set::GuardianSet`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = GuardianSet.$typeName; readonly $fullTypeName: `${typeof PKG_V1}::guardian_set::GuardianSet`; readonly $typeArgs: []; readonly $isPhantom = GuardianSet.$isPhantom;

 readonly index: ToField<"u32">; readonly guardians: ToField<Vector<Guardian>>; readonly expirationTimestampMs: ToField<"u64">

 private constructor(typeArgs: [], fields: GuardianSetFields, ) { this.$fullTypeName = composeSuiType( GuardianSet.$typeName, ...typeArgs ) as `${typeof PKG_V1}::guardian_set::GuardianSet`; this.$typeArgs = typeArgs;

 this.index = fields.index;; this.guardians = fields.guardians;; this.expirationTimestampMs = fields.expirationTimestampMs; }

 static reified( ): GuardianSetReified { return { typeName: GuardianSet.$typeName, fullTypeName: composeSuiType( GuardianSet.$typeName, ...[] ) as `${typeof PKG_V1}::guardian_set::GuardianSet`, typeArgs: [ ] as [], isPhantom: GuardianSet.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => GuardianSet.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => GuardianSet.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => GuardianSet.fromBcs( data, ), bcs: GuardianSet.bcs, fromJSONField: (field: any) => GuardianSet.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => GuardianSet.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => GuardianSet.fromSuiParsedData( content, ), fromSuiObjectData: (content: SuiObjectData) => GuardianSet.fromSuiObjectData( content, ), fetch: async (client: SuiClient, id: string) => GuardianSet.fetch( client, id, ), new: ( fields: GuardianSetFields, ) => { return new GuardianSet( [], fields ) }, kind: "StructClassReified", } }
 ...
 index: this.index,guardians: fieldToJSON<Vector<Guardian>>(`vector<${Guardian.$typeName}>`, this.guardians),expirationTimestampMs: this.expirationTimestampMs.toString(),
 ...
 static fromSuiObjectData( data: SuiObjectData ): GuardianSet { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isGuardianSet(data.bcs.type)) { throw new Error(`object at is not a GuardianSet object`); }

 return GuardianSet.fromBcs( fromB64(data.bcs.bcsBytes) ); } if (data.content) { return GuardianSet.fromSuiParsedData( data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }
