...
export function isVAA(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V1}::vaa::VAA`; }
...
export class VAA implements StructClass { __StructClass = true as const;

 static readonly $typeName = `${PKG_V1}::vaa::VAA`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = VAA.$typeName; readonly $fullTypeName: `${typeof PKG_V1}::vaa::VAA`; readonly $typeArgs: []; readonly $isPhantom = VAA.$isPhantom;
 ...
 private constructor(typeArgs: [], fields: VAAFields, ) { this.$fullTypeName = composeSuiType( VAA.$typeName, ...typeArgs ) as `${typeof PKG_V1}::vaa::VAA`; this.$typeArgs = typeArgs;
 ...
 static reified( ): VAAReified { return { typeName: VAA.$typeName, fullTypeName: composeSuiType( VAA.$typeName, ...[] ) as `${typeof PKG_V1}::vaa::VAA`, typeArgs: [ ] as [], isPhantom: VAA.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => VAA.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => VAA.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => VAA.fromBcs( data, ), bcs: VAA.bcs, fromJSONField: (field: any) => VAA.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => VAA.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => VAA.fromSuiParsedData( content, ), fromSuiObjectData: (content: SuiObjectData) => VAA.fromSuiObjectData( content, ), fetch: async (client: SuiClient, id: string) => VAA.fetch( client, id, ), new: ( fields: VAAFields, ) => { return new VAA( [], fields ) }, kind: "StructClassReified", } }
 ...
 static fromSuiObjectData( data: SuiObjectData ): VAA { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isVAA(data.bcs.type)) { throw new Error(`object at is not a VAA object`); }

 return VAA.fromBcs( fromB64(data.bcs.bcsBytes) ); } if (data.content) { return VAA.fromSuiParsedData( data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }
