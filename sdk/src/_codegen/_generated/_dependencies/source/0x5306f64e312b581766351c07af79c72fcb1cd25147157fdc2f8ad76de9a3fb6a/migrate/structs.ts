...
export function isMigrateComplete(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V1}::migrate::MigrateComplete`; }
...
export class MigrateComplete implements StructClass { __StructClass = true as const;

 static readonly $typeName = `${PKG_V1}::migrate::MigrateComplete`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = MigrateComplete.$typeName; readonly $fullTypeName: `${typeof PKG_V1}::migrate::MigrateComplete`; readonly $typeArgs: []; readonly $isPhantom = MigrateComplete.$isPhantom;

 readonly package: ToField<ID>

 private constructor(typeArgs: [], fields: MigrateCompleteFields, ) { this.$fullTypeName = composeSuiType( MigrateComplete.$typeName, ...typeArgs ) as `${typeof PKG_V1}::migrate::MigrateComplete`; this.$typeArgs = typeArgs;

 this.package = fields.package; }

 static reified( ): MigrateCompleteReified { return { typeName: MigrateComplete.$typeName, fullTypeName: composeSuiType( MigrateComplete.$typeName, ...[] ) as `${typeof PKG_V1}::migrate::MigrateComplete`, typeArgs: [ ] as [], isPhantom: MigrateComplete.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => MigrateComplete.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => MigrateComplete.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => MigrateComplete.fromBcs( data, ), bcs: MigrateComplete.bcs, fromJSONField: (field: any) => MigrateComplete.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => MigrateComplete.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => MigrateComplete.fromSuiParsedData( content, ), fromSuiObjectData: (content: SuiObjectData) => MigrateComplete.fromSuiObjectData( content, ), fetch: async (client: SuiClient, id: string) => MigrateComplete.fetch( client, id, ), new: ( fields: MigrateCompleteFields, ) => { return new MigrateComplete( [], fields ) }, kind: "StructClassReified", } }
 ...
 static fromSuiObjectData( data: SuiObjectData ): MigrateComplete { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isMigrateComplete(data.bcs.type)) { throw new Error(`object at is not a MigrateComplete object`); }

 return MigrateComplete.fromBcs( fromB64(data.bcs.bcsBytes) ); } if (data.content) { return MigrateComplete.fromSuiParsedData( data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }
