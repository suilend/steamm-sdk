...
export function isGuardian(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V1}::guardian::Guardian`; }

export interface GuardianFields { pubkey: ToField<Bytes20> }

export type GuardianReified = Reified< Guardian, GuardianFields >;

export class Guardian implements StructClass { __StructClass = true as const;

 static readonly $typeName = `${PKG_V1}::guardian::Guardian`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = Guardian.$typeName; readonly $fullTypeName: `${typeof PKG_V1}::guardian::Guardian`; readonly $typeArgs: []; readonly $isPhantom = Guardian.$isPhantom;

 readonly pubkey: ToField<Bytes20>

 private constructor(typeArgs: [], fields: GuardianFields, ) { this.$fullTypeName = composeSuiType( Guardian.$typeName, ...typeArgs ) as `${typeof PKG_V1}::guardian::Guardian`; this.$typeArgs = typeArgs;

 this.pubkey = fields.pubkey; }

 static reified( ): GuardianReified { return { typeName: Guardian.$typeName, fullTypeName: composeSuiType( Guardian.$typeName, ...[] ) as `${typeof PKG_V1}::guardian::Guardian`, typeArgs: [ ] as [], isPhantom: Guardian.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => Guardian.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => Guardian.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => Guardian.fromBcs( data, ), bcs: Guardian.bcs, fromJSONField: (field: any) => Guardian.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => Guardian.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => Guardian.fromSuiParsedData( content, ), fromSuiObjectData: (content: SuiObjectData) => Guardian.fromSuiObjectData( content, ), fetch: async (client: SuiClient, id: string) => Guardian.fetch( client, id, ), new: ( fields: GuardianFields, ) => { return new Guardian( [], fields ) }, kind: "StructClassReified", } }
 ...
 static fromSuiObjectData( data: SuiObjectData ): Guardian { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isGuardian(data.bcs.type)) { throw new Error(`object at is not a Guardian object`); }

 return Guardian.fromBcs( fromB64(data.bcs.bcsBytes) ); } if (data.content) { return Guardian.fromSuiParsedData( data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }
