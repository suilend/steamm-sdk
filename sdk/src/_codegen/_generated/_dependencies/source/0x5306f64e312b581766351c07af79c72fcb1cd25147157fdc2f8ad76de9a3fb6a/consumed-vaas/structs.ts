...
export function isConsumedVAAs(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V1}::consumed_vaas::ConsumedVAAs`; }
...
export class ConsumedVAAs implements StructClass { __StructClass = true as const;

 static readonly $typeName = `${PKG_V1}::consumed_vaas::ConsumedVAAs`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = ConsumedVAAs.$typeName; readonly $fullTypeName: `${typeof PKG_V1}::consumed_vaas::ConsumedVAAs`; readonly $typeArgs: []; readonly $isPhantom = ConsumedVAAs.$isPhantom;

 readonly hashes: ToField<Set<ToPhantom<Bytes32>>>

 private constructor(typeArgs: [], fields: ConsumedVAAsFields, ) { this.$fullTypeName = composeSuiType( ConsumedVAAs.$typeName, ...typeArgs ) as `${typeof PKG_V1}::consumed_vaas::ConsumedVAAs`; this.$typeArgs = typeArgs;

 this.hashes = fields.hashes; }

 static reified( ): ConsumedVAAsReified { return { typeName: ConsumedVAAs.$typeName, fullTypeName: composeSuiType( ConsumedVAAs.$typeName, ...[] ) as `${typeof PKG_V1}::consumed_vaas::ConsumedVAAs`, typeArgs: [ ] as [], isPhantom: ConsumedVAAs.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => ConsumedVAAs.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => ConsumedVAAs.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => ConsumedVAAs.fromBcs( data, ), bcs: ConsumedVAAs.bcs, fromJSONField: (field: any) => ConsumedVAAs.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => ConsumedVAAs.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => ConsumedVAAs.fromSuiParsedData( content, ), fromSuiObjectData: (content: SuiObjectData) => ConsumedVAAs.fromSuiObjectData( content, ), fetch: async (client: SuiClient, id: string) => ConsumedVAAs.fetch( client, id, ), new: ( fields: ConsumedVAAsFields, ) => { return new ConsumedVAAs( [], fields ) }, kind: "StructClassReified", } }
 ...
 static fromSuiObjectData( data: SuiObjectData ): ConsumedVAAs { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isConsumedVAAs(data.bcs.type)) { throw new Error(`object at is not a ConsumedVAAs object`); }

 return ConsumedVAAs.fromBcs( fromB64(data.bcs.bcsBytes) ); } if (data.content) { return ConsumedVAAs.fromSuiParsedData( data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }
