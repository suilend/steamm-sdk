...
export class Cursor<T extends TypeArgument> implements StructClass { __StructClass = true as const;

 static readonly $typeName = `${PKG_V1}::cursor::Cursor`; static readonly $numTypeParams = 1; static readonly $isPhantom = [false,] as const;

 readonly $typeName = Cursor.$typeName; readonly $fullTypeName: `${typeof PKG_V1}::cursor::Cursor<${ToTypeStr<T>}>`; readonly $typeArgs: [ToTypeStr<T>]; readonly $isPhantom = Cursor.$isPhantom;

 readonly data: ToField<Vector<T>>

 private constructor(typeArgs: [ToTypeStr<T>], fields: CursorFields<T>, ) { this.$fullTypeName = composeSuiType( Cursor.$typeName, ...typeArgs ) as `${typeof PKG_V1}::cursor::Cursor<${ToTypeStr<T>}>`; this.$typeArgs = typeArgs;

 this.data = fields.data; }

 static reified<T extends Reified<TypeArgument, any>>( T: T ): CursorReified<ToTypeArgument<T>> { return { typeName: Cursor.$typeName, fullTypeName: composeSuiType( Cursor.$typeName, ...[extractType(T)] ) as `${typeof PKG_V1}::cursor::Cursor<${ToTypeStr<ToTypeArgument<T>>}>`, typeArgs: [ extractType(T) ] as [ToTypeStr<ToTypeArgument<T>>], isPhantom: Cursor.$isPhantom, reifiedTypeArgs: [T], fromFields: (fields: Record<string, any>) => Cursor.fromFields( T, fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => Cursor.fromFieldsWithTypes( T, item, ), fromBcs: (data: Uint8Array) => Cursor.fromBcs( T, data, ), bcs: Cursor.bcs(toBcs(T)), fromJSONField: (field: any) => Cursor.fromJSONField( T, field, ), fromJSON: (json: Record<string, any>) => Cursor.fromJSON( T, json, ), fromSuiParsedData: (content: SuiParsedData) => Cursor.fromSuiParsedData( T, content, ), fromSuiObjectData: (content: SuiObjectData) => Cursor.fromSuiObjectData( T, content, ), fetch: async (client: SuiClient, id: string) => Cursor.fetch( client, T, id, ), new: ( fields: CursorFields<ToTypeArgument<T>>, ) => { return new Cursor( [extractType(T)], fields ) }, kind: "StructClassReified", } }
 ...
 static fromSuiObjectData<T extends Reified<TypeArgument, any>>( typeArg: T, data: SuiObjectData ): Cursor<ToTypeArgument<T>> { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isCursor(data.bcs.type)) { throw new Error(`object at is not a Cursor object`); }
