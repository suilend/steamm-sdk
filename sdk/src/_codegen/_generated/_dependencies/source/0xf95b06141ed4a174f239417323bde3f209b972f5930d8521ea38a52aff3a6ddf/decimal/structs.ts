...
export interface DecimalFields { value: ToField<"u256"> }
...
export class Decimal implements StructClass { __StructClass = true as const;
 ...
 static fromFields( fields: Record<string, any> ): Decimal { return Decimal.reified( ).new( { value: decodeFromFields("u256", fields.value) } ) }

 static fromFieldsWithTypes( item: FieldsWithTypes ): Decimal { if (!isDecimal(item.type)) { throw new Error("not a Decimal type");

 }

 return Decimal.reified( ).new( { value: decodeFromFieldsWithTypes("u256", item.fields.value) } ) }
 ...
 static fromJSONField( field: any ): Decimal { return Decimal.reified( ).new( { value: decodeFromJSONField("u256", field.value) } ) }
