...
export class StorageFund implements StructClass { __StructClass = true as const;
 ...
 static fromSuiObjectData( data: SuiObjectData ): StorageFund { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isStorageFund(data.bcs.type)) { throw new Error(`object at is not a StorageFund object`); }

 return StorageFund.fromBcs( fromB64(data.bcs.bcsBytes) ); } if (data.content) { return StorageFund.fromSuiParsedData( data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }
