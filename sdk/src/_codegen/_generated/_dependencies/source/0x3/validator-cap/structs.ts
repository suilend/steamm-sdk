...
export class UnverifiedValidatorOperationCap implements StructClass { __StructClass = true as const;
 ...
 static fromSuiObjectData( data: SuiObjectData ): UnverifiedValidatorOperationCap { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isUnverifiedValidatorOperationCap(data.bcs.type)) { throw new Error(`object at is not a UnverifiedValidatorOperationCap object`); }

 return UnverifiedValidatorOperationCap.fromBcs( fromB64(data.bcs.bcsBytes) ); } if (data.content) { return UnverifiedValidatorOperationCap.fromSuiParsedData( data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }
...
export class ValidatorOperationCap implements StructClass { __StructClass = true as const;
 ...
 static fromSuiObjectData( data: SuiObjectData ): ValidatorOperationCap { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isValidatorOperationCap(data.bcs.type)) { throw new Error(`object at is not a ValidatorOperationCap object`); }

 return ValidatorOperationCap.fromBcs( fromB64(data.bcs.bcsBytes) ); } if (data.content) { return ValidatorOperationCap.fromSuiParsedData( data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }
