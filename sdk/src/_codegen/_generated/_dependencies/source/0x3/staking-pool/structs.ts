...
export class PoolTokenExchangeRate implements StructClass { __StructClass = true as const;
 ...
 static fromSuiObjectData( data: SuiObjectData ): PoolTokenExchangeRate { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isPoolTokenExchangeRate(data.bcs.type)) { throw new Error(`object at is not a PoolTokenExchangeRate object`); }

 return PoolTokenExchangeRate.fromBcs( fromB64(data.bcs.bcsBytes) ); } if (data.content) { return PoolTokenExchangeRate.fromSuiParsedData( data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }
...
export class StakedSui implements StructClass { __StructClass = true as const;
 ...
 static fromSuiObjectData( data: SuiObjectData ): StakedSui { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isStakedSui(data.bcs.type)) { throw new Error(`object at is not a StakedSui object`); }

 return StakedSui.fromBcs( fromB64(data.bcs.bcsBytes) ); } if (data.content) { return StakedSui.fromSuiParsedData( data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }
...
export class StakingPool implements StructClass { __StructClass = true as const;
 ...
 static fromSuiObjectData( data: SuiObjectData ): StakingPool { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isStakingPool(data.bcs.type)) { throw new Error(`object at is not a StakingPool object`); }

 return StakingPool.fromBcs( fromB64(data.bcs.bcsBytes) ); } if (data.content) { return StakingPool.fromSuiParsedData( data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }
