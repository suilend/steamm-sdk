...
export function isStakeSubsidy(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V18}::stake_subsidy::StakeSubsidy`; }
...
export class StakeSubsidy implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `${PKG_V18}::stake_subsidy::StakeSubsidy`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = StakeSubsidy.$typeName; readonly $fullTypeName: `${typeof PKG_V18}::stake_subsidy::StakeSubsidy`; readonly $typeArgs: []; readonly $isPhantom = StakeSubsidy.$isPhantom;
 ...
 private constructor(typeArgs: [], fields: StakeSubsidyFields, ) { this.$fullTypeName = composeSuiType( StakeSubsidy.$typeName, ...typeArgs ) as `${typeof PKG_V18}::stake_subsidy::StakeSubsidy`; this.$typeArgs = typeArgs;
 ...
 static fromSuiObjectData( data: SuiObjectData ): StakeSubsidy { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isStakeSubsidy(data.bcs.type)) { throw new Error(`object at is not a StakeSubsidy object`); }

 return StakeSubsidy.fromBcs( fromB64(data.bcs.bcsBytes) ); } if (data.content) { return StakeSubsidy.fromSuiParsedData( data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }
