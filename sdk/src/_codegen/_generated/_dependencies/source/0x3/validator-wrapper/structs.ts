...
export class ValidatorWrapper implements StructClass { __StructClass = true as const;
 ...
 static phantom( ): PhantomReified<ToTypeStr<ValidatorWrapper>> { return phantom(ValidatorWrapper.reified( )); } static get p() { return ValidatorWrapper.phantom() }

 static get bcs() { return bcs.struct("ValidatorWrapper", {

 inner: Versioned.bcs

}) };

 static fromFields( fields: Record<string, any> ): ValidatorWrapper { return ValidatorWrapper.reified( ).new( { inner: decodeFromFields(Versioned.reified(), fields.inner) } ) }
 ...
 return ValidatorWrapper.reified( ).new( { inner: decodeFromFieldsWithTypes(Versioned.reified(), item.fields.inner) } ) }

 static fromBcs( data: Uint8Array ): ValidatorWrapper { return ValidatorWrapper.fromFields( ValidatorWrapper.bcs.parse(data) ) }
 ...
 static fromJSONField( field: any ): ValidatorWrapper { return ValidatorWrapper.reified( ).new( { inner: decodeFromJSONField(Versioned.reified(), field.inner) } ) }

 static fromJSON( json: Record<string, any> ): ValidatorWrapper { if (json.$typeName !== ValidatorWrapper.$typeName) { throw new Error("not a WithTwoGenerics json object") };

 return ValidatorWrapper.fromJSONField( json, ) }

 static fromSuiParsedData( content: SuiParsedData ): ValidatorWrapper { if (content.dataType !== "moveObject") { throw new Error("not an object"); } if (!isValidatorWrapper(content.type)) { throw new Error(`object at ${(content.fields as any).id} is not a ValidatorWrapper object`); } return ValidatorWrapper.fromFieldsWithTypes( content ); }

 static fromSuiObjectData( data: SuiObjectData ): ValidatorWrapper { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isValidatorWrapper(data.bcs.type)) { throw new Error(`object at is not a ValidatorWrapper object`); }

 return ValidatorWrapper.fromBcs( fromB64(data.bcs.bcsBytes) ); } if (data.content) { return ValidatorWrapper.fromSuiParsedData( data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }

 static async fetch( client: SuiClient, id: string ): Promise<ValidatorWrapper> { const res = await client.getObject({ id, options: { showBcs: true, }, }); if (res.error) { throw new Error(`error fetching ValidatorWrapper object at id ${id}: ${res.error.code}`); } if (res.data?.bcs?.dataType !== "moveObject" || !isValidatorWrapper(res.data.bcs.type)) { throw new Error(`object at id ${id} is not a ValidatorWrapper object`); }
